"""
Syncs plumbing leads into the "Plumbing" tab of the unified Outreach Tracker sheet.
...
Usage:
  python3 tools/log_plumbing_outreach.py           # sync drafted leads, print sheet URL
  python3 tools/log_plumbing_outreach.py --url     # print sheet URL only
"""
import json
import sys
...
LEADS_JSON = ROOT / ".tmp" / "plumbing_leads.json"
...
def main():
    ...
    if not LEADS_JSON.exists():
        print(f"No leads found at {LEADS_JSON}. Run scrape_plumbing_gmaps.py first.")
        sys.exit(1)
    ...
    added = tracker.add_rows("Plumbing", rows)
    if added:
        print(f"Added {added} new leads to Outreach Tracker → Plumbing.")
    else:

#!/usr/bin/env python3
"""
Read the Talento USA content calendar (Google Sheet) and publish any pending posts that are due.
Updates each row's Status → "posted" and records the Post ID.

Usage:
  python3 tools/auto_post_talento_usa.py            # post all due pending items
  python3 tools/auto_post_talento_usa.py --dry-run  # show what would be posted
  python3 tools/auto_post_talento_usa.py --force    # post regardless of scheduled time
"""
...
def publish(row: list, dry_run: bool):
    ...
    env = os.environ.copy()
    env["INSTAGRAM_BUSINESS_ACCOUNT_ID"] = os.getenv("TALENTO_USA_INSTAGRAM_BUSINESS_ACCOUNT_ID", "")
...
def main():
    ...
    ig_id = os.getenv("TALENTO_USA_INSTAGRAM_BUSINESS_ACCOUNT_ID")
    if not ig_id:
        print("ERROR: TALENTO_USA_INSTAGRAM_BUSINESS_ACCOUNT_ID not set in .env")
        sys.exit(1)
    ...
    print(f"[Talento USA] Checking calendar — {now.strftime('%Y-%m-%d %H:%M')} AR time")
    ...
    for i, row, post_dt in batch:
        ...
        if args.dry_run:
            notify(account="talento_usa", caption=caption, image_url=notify_image, hashtags=hashtags, status="dry-run")
        elif post_id:
            update_status(sheets, sheet_id, i, "posted", post_id)
            print(f"    Sheet updated → posted (ID: {post_id})")
            posted += 1
            notify(account="talento_usa", caption=caption, image_url=notify_image, hashtags=hashtags, status="posted", post_id=post_id)
        else:
            update_status(sheets, sheet_id, i, "error", "")
            print("    Sheet updated → error")
            notify(account="talento_usa", caption=caption, image_url=notify_image, hashtags=hashtags, status="error")
        print()

"""
Generates personalized cold-outreach Gmail drafts for window replacement leads.

Reads from .tmp/windows_leads.json, picks leads with email addresses,
and creates HTML drafts in Gmail tagged Label_13 (Window - Lead Flow Angle).

Usage:
  python3 tools/generate_windows_drafts.py              # create drafts for leads without one
  python3 tools/generate_windows_drafts.py --rebuild    # delete existing drafts and recreate all
  python3 tools/generate_windows_drafts.py --limit 5   # process first 5 only
  python3 tools/generate_windows_drafts.py --min-tier A  # only Tier A leads (default: B)
"""
...
LEADS_JSON = os.path.join(TMP_DIR, "windows_leads.json")

LABEL_ID = "Label_13"  # Window - Lead Flow Angle

TEMPLATES = {
    "none": (
        "Quick question about your window leads in {city}",
        "Hey {name},\n\n"
        "Found your window company in {city} and noticed you don't have any ads running — "
        "most of your competitors are already using Meta and Google to lock up homeowners "
        "before they even start getting quotes.\n\n"
        "We run paid ads exclusively for window replacement contractors. Last month we helped "
        "a company in a nearby market go from 6 estimates/week to 28 — all exclusive leads, "
        "no HomeAdvisor, no Angi, no sharing the same homeowner with 4 other window companies.\n\n"
        "Would it make sense to hop on a 15-min call this week to see if there's a fit? "
        "No pitch, just want to understand what your install schedule looks like.\n\n"
        "— Guido"
    ),
    "active": (
        "Are your window leads exclusive or shared?",
        "Hey {name},\n\n"
        "Noticed you're running some ads — good move. Most window companies in your market aren't.\n\n"
        "Quick question: are your leads exclusive, or are they going to 3–5 other window contractors "
        "at the same time?\n\n"
        "We run campaigns where every lead comes only to you. Higher close rate, less time chasing "
        "homeowners who already signed with someone else.\n\n"
    ...
    "inactive": (
        ...
        "story I hear from window company owners: ran ads, got garbage leads, or the agency "
        "disappeared after month 2.\n\n"
        "We only work with window replacement contractors and we guarantee exclusive leads — "
        "the same homeowner doesn't go to 5 other companies in your area.\n\n"
...
def run(limit: int = None, rebuild: bool = False, min_tier: str = "B"):
    ...
    if unscored:
        print(f"  Warning: {len(unscored)} lead(s) have no tier — run score_leads.py --niche windows first.")

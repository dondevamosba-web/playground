"""
Generates personalized cold-outreach Gmail drafts for roofing leads.

Reads from .tmp/roofing_leads.json, picks leads with email addresses,
and creates HTML drafts in the dondevamosba Gmail account.

Usage:
  python3 tools/generate_roofing_drafts.py              # create drafts for leads without one
  python3 tools/generate_roofing_drafts.py --rebuild    # delete existing drafts and recreate all
  python3 tools/generate_roofing_drafts.py --limit 5   # process first 5 only
  python3 tools/generate_roofing_drafts.py --min-tier A  # only Tier A leads (default: B)
"""
...
LEADS_JSON = os.path.join(TMP_DIR, "roofing_leads.json")

TEMPLATES = {
    "never": (
        "Quick question about your roofing leads in {city}",
        "Hey {name},\n\n"
        "Found your business in {city} and noticed you don't have any ads running — "
        "most of your competitors are already using Meta and Google to lock up local leads.\n\n"
        "We run paid ads exclusively for roofing contractors. Last month we helped a company "
        "in a nearby market go from 3 leads/week to 18 — all exclusive, no HomeAdvisor, no lead-sharing.\n\n"
        "Would it make sense to hop on a 15-min call this week to see if there's a fit? "
        "No pitch, just want to understand what your busy season looks like."
    ),
    "inactive": (
        "You ran Facebook ads before — here's what went wrong",
        "Hey {name},\n\n"
        "Saw you ran some Facebook ads a while back but stopped. That's actually the most common "
        "story I hear from roofers: ran ads, got garbage leads, or the agency disappeared after month 2.\n\n"
        "We only work with roofing companies (no landscapers, plumbers, etc.) and we guarantee "
        "exclusive leads — the same lead doesn't go to 5 other roofers in your area.\n\n"
    ...
    "active": (
        "Are your roofing leads exclusive or shared?",
        "Hey {name},\n\n"
        "Noticed you're running some ads — good move. Most roofing companies in your market aren't.\n\n"
        "Quick question: are your leads exclusive, or are they going to 3–5 other contractors at the same time?\n\n"
        "We run campaigns where every lead comes only to you. Higher close rate, less wasted time "
        "chasing people who already booked someone else.\n\n"
        "Happy to do a free audit of what you're running now and show you exactly where leads are "
        "leaking. No strings attached."
    ),
}
DEFAULT_TEMPLATE = "never"
...
def run(limit: int = None, rebuild: bool = False, min_tier: str = "B"):
    ...
    for i, lead in enumerate(filtered, 1):
        ...
        status = lead.get("fb_ads_status", "none")

        template_key = status if status in TEMPLATES else DEFAULT_TEMPLATE
        ...
        try:
            result = create_draft(
                ...
                body=html_body,
                html=True,
                label_ids=["Label_10"],  # Roofing - Lead Flow Angle
            )

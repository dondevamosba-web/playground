#!/usr/bin/env python3
"""
Read the Ola Empleo content calendar (Google Sheet) and publish any pending posts that are due.
Updates each row's Status → "posted" and records the Post ID.

Usage:
  python3 tools/auto_post_ola_empleo.py            # post all due pending items
  python3 tools/auto_post_ola_empleo.py --dry-run  # show what would be posted
  python3 tools/auto_post_ola_empleo.py --force    # post regardless of scheduled time
"""
...
def publish(row: list, dry_run: bool):
    ...
    # Local paths aren't publicly reachable — Instagram's API needs a URL.
    # Missing here until 2026-07-21 (found while testing a live post: this
    # was about to hand post_instagram.py a filesystem path and fail).
    # Same upload pattern as auto_post_from_calendar.py.
    ...
    env = os.environ.copy()
    env["INSTAGRAM_BUSINESS_ACCOUNT_ID"] = os.getenv("OLA_EMPLEO_INSTAGRAM_BUSINESS_ACCOUNT_ID", "")
...
def main():
    ...
    ig_id = os.getenv("OLA_EMPLEO_INSTAGRAM_BUSINESS_ACCOUNT_ID")
    if not ig_id:
        print("ERROR: OLA_EMPLEO_INSTAGRAM_BUSINESS_ACCOUNT_ID not set in .env")
        sys.exit(1)
    ...
    print(f"[Empleo] Checking calendar — {now.strftime('%Y-%m-%d %H:%M')} AR time")
    ...
    for i, row, post_dt in batch:
        ...
        if args.dry_run:
            notify(account="olavarria_empleo", caption=caption, image_url=notify_image, hashtags=hashtags, status="dry-run")
        elif post_id:
            update_status(sheets, sheet_id, i, "posted", post_id)
            print(f"    Sheet updated → posted (ID: {post_id})")
            posted += 1
            notify(account="olavarria_empleo", caption=caption, image_url=notify_image, hashtags=hashtags, status="posted", post_id=post_id)
        else:
            update_status(sheets, sheet_id, i, "error", "")
            print("    Sheet updated → error")
            notify(account="olavarria_empleo", caption=caption, image_url=notify_image, hashtags=hashtags, status="error")
        print()

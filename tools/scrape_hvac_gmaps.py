"""
Scrapes Google Maps for small HVAC contractors in US cities.
Extracts: name, address, phone, website, rating, review count.

"Small" = fewer than 100 reviews (proxy for size).

Usage:
  python3 tools/scrape_hvac_gmaps.py                     # default cities
  python3 tools/scrape_hvac_gmaps.py --cities "Austin TX,Denver CO"
  python3 tools/scrape_hvac_gmaps.py --limit 50          # max results per city
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "..", ".tmp", "hvac_leads.json")
...
SMALL_THRESHOLD = 100  # max reviews to be considered "small"

# HVAC-specific search terms — rotate across cities for broader coverage
SEARCH_QUERIES = [
    "HVAC contractor",
    "AC repair company",
...
def scrape_city(page, city: str, limit: int) -> list[dict]:
    ...
    while len(results) < limit and scroll_attempts < max_scrolls:
        ...
        for card in cards:
            ...
            try:
                ...
                # Skip if too many reviews (not a small company)
                ...
                results.append({
                    ...
                    "phone": None,       # populated later via detail page
                    "website": None,     # populated later via detail page
                    "fb_ads_status": None,  # populated by check_fb_ads.py
                })
            except Exception:
                continue
    ...
    # Enrich top results with phone + website from detail pages
...
def main():
    ...
    print(f"Scraping {len(cities)} cities, up to {args.limit} small HVAC companies each...")

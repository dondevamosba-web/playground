"""
Scrapes Google Maps for small roofing contractors in US cities.
Extracts: name, address, phone, website, rating, review count.

"Small" = fewer than 100 reviews (proxy for size).

Usage:
  python3 tools/scrape_roofing_gmaps.py                     # default cities
  python3 tools/scrape_roofing_gmaps.py --cities "Austin TX,Denver CO"
  python3 tools/scrape_roofing_gmaps.py --limit 50          # max results per city
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "..", ".tmp", "roofing_leads.json")
...
def scrape_city(page, city: str, limit: int) -> list[dict]:
    query = f"roofing contractor {city}"
...
def main():
    ...
    print(f"Scraping {len(cities)} cities, up to {args.limit} small roofers each...")
